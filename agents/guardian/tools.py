...
async def get_medication_schedule(tool_context=None) -> dict:
    """Get today's medication schedule for the patient.

    Returns the full list of medications with names, dosages, times,
    and whether each dose has been taken today.
    """
    ...
    if _use_firestore(tool_context):
        ...
        adherence = await fs.get_adherence_log(user_id, since_date=today)
    else:
...
async def log_medication_schedule(
    medication_name: str, 
    schedule_type: str, 
    dose_times: list[str], 
    rxnorm_id: str = "", 
    ...
    if _use_firestore(tool_context):
        fs = FirestoreService.get_instance()
        try:
            await fs.add_medication(user_id, medication_name, schedule_type, dose_times, rxnorm_id)
...
async def log_medication_taken(medication_name: str, tool_context=None) -> dict:
    ...
    if _use_firestore(tool_context):
        await fs.add_adherence_entry(user_id, entry)
    else:
...
async def log_vitals(
    ...
    if _use_firestore(tool_context):
        user_id = _get_user_id(tool_context)
        fs = FirestoreService.get_instance()
        await fs.add_vitals_entry(user_id, entry)
    else:
...
async def log_meal(
    ...
    if _use_firestore(tool_context):
        user_id = _get_user_id(tool_context)
        fs = FirestoreService.get_instance()
        await fs.add_meals_entry(user_id, entry)
    else:
...
async def detect_emergency_severity(user_message: str, tool_context=None) -> dict:
...
async def initiate_emergency_protocol(
    ...
    if _use_firestore(tool_context):
        fs = FirestoreService.get_instance()
        try:
            await fs.add_emergency_incident(user_id, incident)
        except Exception:
            pass
    else:
...
async def initiate_family_call(
    ...
    if not twilio_sid or not twilio_token or not twilio_from:
        ...
        if _use_firestore(tool_context):
            try:
                await fs.add_call_log(user_id, call_log)
            except Exception:
                pass
        else:
