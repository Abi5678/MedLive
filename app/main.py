...
@app.get("/")
async def index():
    """Serve the parent-facing voice UI."""
...
async def dashboard_page():
    """Serve the family caregiver health dashboard."""
        ...
        speech_config=types.SpeechConfig(
            voice_config=types.VoiceConfig(
                prebuilt_voice_config=types.PrebuiltVoiceConfig(
                    voice_name=voice_name,  # Dynamic voice from Profile
                )
            )
        ),
    )
            ...
            while True:
                ...
                if msg_type == "audio":
                    ...
                    if red_line_hit:
                        ...
                        emergency_content = types.Content(
                            parts=[types.Part(text=directive)]
                        )
                        live_request_queue.send_content(emergency_content)
